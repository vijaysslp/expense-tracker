/* ExpensePro v7++ — Full script (Gmail + CSV/XLSX + Charts + Dedupe + Private Mappings + Card Parsers)
   - Robust Google sign-in (uses window.GOOGLE_CLIENT_ID from index.html)
   - Gmail scan: generic + issuer-domain queries
   - Strong noise filters (OTP/declined) without dropping legit alerts
   - Issuer templates (SBI/HDFC/Axis/ICICI) for accurate amount/card/merchant/type
   - Fingerprint dedupe (amount+card+merchant+time-bucket), persisted across rescans
   - CSV/XLSX import/export
   - Private JSON mappings (cards/accounts/merchant/category rules) with optional AES-GCM encryption
   - Dashboard + charts
*/
...
// widen search to typical issuers/gateways
const BANKS = [
...
// issuer email domains (card alerts)
const CARD_SENDERS = [
...
$("#btn-export")?.addEventListener("click",()=>{
  const tx=fTx(); if(!tx.length) return alert("No transactions to export.");
...
// ---------- CSV/XLSX or JSON (mappings) IMPORT ----------
$("#fileInput")?.addEventListener("change", async e=>{
  ...
  try{
    ...
    if(name.endsWith(".json")){
      ...
      mergeCsv(await f.text()); log(`Imported ${f.name}`); renderAll();
    }else{
...
function ingestRow(r){
  ...
  let type=(r.type??r.Type??r.TYPE??(amt<0?"credit":"debit")).toString().toLowerCase().includes("credit")?"credit":"debit";
...
function mergeCsv(text){
  ...
  const iD=idx("date"), iA=idx("amount"), iT=idx("type"), iM=idx("merchant"), iC=idx("card"), iCat=idx("category");
  L.forEach(line=>{
...
// ---------- PRIVATE MAPPINGS (LOCAL + optional encryption) ----------
/*
  mappings.json schema example:
  {
    "cards":[{"match":"3183|SBI.*3183","label":"SBI Credit Card • 3183"}],
    "accounts":[{"match":"A\\/c\\s*1234|XXXX1234","label":"HDFC Savings • 1234"}],
    "merchantRules":[{"pattern":"HPCL|IOCL|BPCL|FILLING\\s*STATION","merchant":"Fuel Pump","category":"Fuel"}],
    "categoryRules":[{"pattern":"electric|tata\\s*power|adani|cesc|kseb|mseb","category":"Utilities"}]
  }
*/
...
function loadMappingsFromLocal(){
  try{
    ...
    if(obj.enc) return {cards:[],accounts:[],merchantRules:[],categoryRules:[], _encrypted:true};
...
async function importMappingsJSON(text){
  ...
  obj.cards=obj.cards||[]; obj.accounts=obj.accounts||[];
  obj.merchantRules=obj.merchantRules||[]; obj.categoryRules=obj.categoryRules||[];
  userMap=obj;
  const pass=$("#passphrase")?.value?.trim();
  if(pass) await encSave(pass,obj);
  else localStorage.setItem(LS_KEY_MAP, JSON.stringify(obj));
  log(`Imported mappings: ${obj.cards.length} card, ${obj.accounts.length} account, ${obj.merchantRules.length} merchant, ${obj.categoryRules.length} category rules`);
}
...
const STOP_PATTERNS = [
  ...
  // NOTE: do NOT include "inform you that" — legit HDFC alerts use it
...
async function scanGmail(){
  ...
  // 2) issuer-domain specific
  for(const d of CARD_SENDERS){
    const q = `newer_than:365d ${exclude} (from:${d} ${base})`;
    await scanQuery(q, 200);
  }

  if(!state.tx.length) log("No results. Try CSV import or check Gmail access.");
  renderAll();
}
...
function detectMerchant(text){
  let m=text.match(/(?:towards|at|merchant\s*[:\-])\s*([A-Z0-9& ._\-]{2,60})\b/i);
...
function categorize(t){
  ...
  if(/(electric|bijli|tata\s*power|adani|b(es?)c|cesc|kseb|mseb|pspcl|apdcl)/.test(x)) return "Utilities"; // electricity
...
function parseSbiCard(text){
  const m=text.match(/SBI\s*Card.*?(?:INR|Rs\.?|₹)\s*([0-9][0-9,]*(?:\.\d{1,2})?).*?(?:spent|debited|credited).*?Credit\s*Card.*?(?:ending|xx|xxxx)\s*([0-9]{3,6}|[0-9]{4}).*?(?:at|towards)\s+([A-Z0-9& ._\-]{2,60})/i);
  if(!m) return null;
  return {amount:parseFloat(m[1].replace(/,/g,'')), card:m[2], merchant:m[3].trim(), type:/credited|refund/i.test(text)?"credit":"debit"};
}
...
function parseCardAlert(text){
  return parseSbiCard(text) || parseHdfcCard(text) || parseAxisCard(text) || parseIciciCard(text) || null;
}
...
function processMsg(msg){
  ...
  // issuer-specific parse (preferred)
  const cardHit=parseCardAlert(text);
  if(cardHit && cardHit.amount>0){
    const tx={
      id:msg.id, date,
      amount:cardHit.amount, type:cardHit.type,
      ...
      source:"gmail", raw:text
    };
    applyUserMappings(tx);
    const fp=makeFingerprint(tx); if(fpSeen.has(fp)) return false; saveFp(fp);
    state.tx.push(tx); return true;
  }
...
function fTx(){
  ...
  const typ=typeEl?.value||"all"; const card=(cardEl?.value||"").toLowerCase();
  const cat=catEl?.value||"all"; const q=(qEl?.value||"").toLowerCase();
...
function drawCharts(tx){
  ...
  for(let i=11;i>=0;i--){
    ...
    months.push({label:d.toLocaleString(undefined,{month:'short'}), s:+new Date(d.getFullYear(),d.getMonth(),1), e:+new Date(d.getFullYear(),d.getMonth()+1,1)-1});
  }
